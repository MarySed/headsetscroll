...
export default function Model({ scroll, ...props }) {
  ...
  useFrame((state) => {
    actions["CameraAction.005"].time = THREE.MathUtils.lerp(actions["CameraAction.005"].time, actions["CameraAction.005"].getClip().duration * scroll.current, 0.1);

    group.current.children[0].children.forEach((child, index) => {
      child.material.color.lerp(color.set(hovered === child.name ? "gold" : "#202020").convertSRGBToLinear(), hovered ? 0.02 : 0.05);

      const elapsedTime = state.clock.elapsedTime;

      child.position.y = Math.sin((elapsedTime + index * 2000) / 2) * 1;
      child.rotation.x = Math.sin((elapsedTime + index * 2000) / 3) / 10;
      child.rotation.y = Math.cos((elapsedTime + index * 2000) / 2) / 10;
      child.rotation.z = Math.sin((elapsedTime + index * 2000) / 3) / 10;
    });
  });
